...
import { Loader2, FileText, Download, Printer, BarChart3 } from 'lucide-react'
import { ProgressChart, SingleGoalProgressChart } from './progress-chart'
...
export function ProgressReportGenerator({
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      // Fetch session goals with session dates for charting
      const sessionIds = (sessions as { id: string; session_date: string }[] | null)?.map((s) => s.id) || []
      const sessionDateMap = (sessions as { id: string; session_date: string }[] | null)?.reduce((acc, s) => {
        acc[s.id] = s.session_date
        return acc
      ...
      if (sessionIds.length > 0) {
        ...
        sessionGoals = ((sg as { session_id: string; goal_id: string; progress_value: number | null; progress_unit: string | null; notes: string | null }[]) || []).map((item) => ({
          ...item,
          session_date: sessionDateMap[item.session_id] || '',
        }))
      }
      ...
      {reportData && (
        <div className="border rounded-lg p-6 bg-white print:border-none print:p-0">
          ...
          <div className="mb-8 print:break-inside-avoid">
            ...
            {reportData.goals.length === 0 || reportData.sessionGoals.length === 0 ? (
              ...
              <div className="border rounded-lg p-4 bg-white">
                <ProgressChart
                  ...
                  progressData={reportData.sessionGoals}
                  height={300}
                />
              </div>
            )}
          ...
          {reportData.goals.length > 0 && reportData.sessionGoals.length > 0 && (
            <div className="mb-8 print:break-inside-avoid">
              <h2 className="font-semibold text-lg mb-4">Individual Goal Progress</h2>
