...
export async function GET(request: NextRequest) {
  ...
  if (studentIds.length > 0) {
    const { data: goals } = await supabase
      ...
      .in('student_id', studentIds as string[]) as {
  ...
  if (studentIds.length > 0) {
    const { data: sessions } = await supabase
      ...
      .gte('session_date', startOfMonth.toISOString()) as {
        data: Array<{ student_id: string; duration_minutes: number | null; session_date: string }> | null
      }
